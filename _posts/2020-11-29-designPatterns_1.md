...
· 提供一个一系列相关或相互依赖的接口，无需指定他们具体的类。
· 属于创建型设计模式。
[点这里看菜鸟的介绍](https://www.runoob.com/design-pattern/abstract-factory-pattern.html)
假如现在要实现这几个接口负责的功能：
```
public interface ICourse {
    void record();
...
```
总结：
· 以上就是抽象工厂的一个综合，虽然他的产品结构变得更加复杂，但其符合类的单一职责原则；
· 完整的描述了Java和Python两个产品等级的关系；
· 扩展费力，每一层都要修改
抽象工厂模式的缺点：
1. 规定了所有可能被创建的集合，产品族中扩展新的产品困难，需要修改抽象工厂的接口；
2. 增加了系统的抽象性和理解难度。
抽象工厂模式的优点：
...
3. 提供一个产品类的库，所有的产品以同样的接口出现从而使客户端不依赖于具体的实现。
